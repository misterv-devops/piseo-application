...
namespace InstrumentSystems.CAS4
{
    ...
    public class CAS4DLL
    {
        ...
        public const int dpidCheckReferenceSpectrumFile = 162; ///<string, write-only: pass the complete file path of an ACS reference spectrum to check whether that spectrum has matching settings, so it can be compared with spectra of this CAS ID. Returns <see cref="ErrorInvalidCalibration"/> if the spectrum file could not be loaded, <see cref="ErrorInvalidParameter"/> if the settings of the spectrum do not match that of the CAS ID and <see cref="ErrorNoError"/> if they do match.
